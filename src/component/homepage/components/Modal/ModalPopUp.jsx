...
function ModalPopUp() {
  ...
  useEffect(() => {
    ...
    let productDetailId = modalState.productItem.productDetailId
      ? modalState.productItem.productDetailId
      : [];
    productDetailId.forEach((item) => {
      if (!dataRenderBuffer[item.option[0].value]) {
        ...
        dataRenderBuffer[item.option[0].value].listSize = [
          item.option[1].value,
        ];
      } else {
        dataRenderBuffer[item.option[0].value].listSize.push(
          item.option[1].value
        );
      }
    });
          ...
          <Row gutter={[10, 10]}>
            ...
            <Col span={12}>
              <div className={cx("product-info-popup")}>
                ...
                <p className={cx("product-price-popup")}>
                  Giá :
                  {currentOption
                    ? currentOption.price?.toLocaleString()
                    : null || modalState.productItem.price?.toLocaleString()}
                </p>
                  ...
                  <>
                        ...
                        {dataRender.map((item, index) => {
                          return (
                            <div
                              key={index}
                              className={
                                item[0] === currentOption.color
                                  ? cx("product-color-active")
                                  : cx("product-color")
                              }
                              onClick={() => handleColorProduct(item)}>
                              <img
                                className={cx("img-product-color")}
                                src={item[1].listImg[0]}
                                alt=""
                                width="50%"
                              />
                              <img
                                className={
                                  item[0] === currentOption.color
                                    ? cx("img-active-color")
                                    : cx("img-not-active-color")
                                }
                                src="https://theme.hstatic.net/1000205427/1000509844/14/select-pro.png?v=56"
                                alt=""
                              />
                              <p className={cx("name-color-product")}>
                                {item[0]}
                              </p>
                            </div>
                          );
                    ...
                    <div className={cx("product-size-box-popup")}>
                      ...
                      <div className={cx("product-size-box")}>
                        {currentListSize
                          ? currentListSize.map((item, index) => {
                              return (
                                <div
                                  key={Math.random()}
                                  className={
                                    item === currentOption.size
                                      ? cx("product-size-active")
                                      : cx("product-size")
                                  }
                                  onClick={() => handleSizeProduct(item)}>
                                  <img
                                    className={
                                      item === currentOption.size
                                        ? cx("img-active-size")
                                        : cx("img-not-active-size")
                                    }
                                    src="https://theme.hstatic.net/1000205427/1000509844/14/select-pro.png?v=56"
                                    alt=""
                                  />
